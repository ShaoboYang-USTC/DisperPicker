...
path = os.path.split(root_path)[0] + '/pick_result/group_velocity'
os.system('rm -rf %s/new/group_velocity/*'%root_path)
...
downward = 0.3                 # The velocity of the curve must decreases less than 0.3 km/s from short to long period. 
each_stage_downward = 0.2      # Each of the valid stage of the curve must decreases less than 0.2 km/s.
max_diff = 0.08                 # v[i+1] - v[i] <= max_diff
min_diff = -0.07                # v[i+1] - v[i] >= min_diff
    ...
    for k in range(len(stage_len)):
        ...
        if stage_len[k] >= min_len:
            ...
            if len(true_length) > 0:
                ...
                final_velocity[true_start[max_index]:true_end[max_index]+1] = valid_velocity[true_start[max_index]:
                                                                                                true_end[max_index]+1]
                ...
                if (true_length[max_index] >= min_len and 
                    ...
                    np.savetxt(root_path + '/new/group_velocity/' + dir_name[i], write_G, fmt="%1.2f  %1.2f")
                    break
...
print('remaining group velocity number: ',all_group_velocity_num)
